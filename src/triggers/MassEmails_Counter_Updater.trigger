trigger MassEmails_Counter_Updater on xtma_Individual_Email_Result__c (before insert) {
    ...
    for (Lead l:leadsToUpdate){
        for (Campaign c: CampaignToQuery) {
            ...
            if (c.Type == 'Regional Email') {
                if(l.Number_Regional_Emails__c == NULL) {
                    l.Number_Regional_Emails__c =1;
                    l.Regional_Email_Sent_Date__c = System.Now();
                   }               
                else {
                    l.Number_Regional_Emails__c ++;
                    l.Regional_Email_Sent_Date__c = System.Now();
                    }
                }       
